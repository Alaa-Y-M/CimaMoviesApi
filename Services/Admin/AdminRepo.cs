...
using CimaMoviesApi.Dtos.users;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;

namespace CimaMoviesApi.Services.Admin;
public class AdminRepo : IAdminRepository
{
    ...
    [Obsolete]
    public AdminRepo(ApplicationDbContext db, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, IWebHostEnvironment host)
    {
        _db = db;
    ...
    public async Task<ApplicationUser> EditUserAsync(EditUserModel model)
    {
        ...
        if (model.Password != user.PasswordHash)
        {
            ...
            if (result.Succeeded)
            {
                await _userManager.AddPasswordAsync((ApplicationUser)user, model.Password);
            }
        }
    ...
    {
        return (IEnumerable<ApplicationUser>)await _db.Users.ToListAsync();
    }
    ...
    public async Task<bool> EditUserRoleAsync(EditUserRoleModel model)
    {
        ...
        var currentRoleName = await _roleManager.Roles.Where(x => x.Id == currentRoleId).Select(x => x.Name).FirstOrDefaultAsync();
        var newRoleName = await _roleManager.Roles.Where(x => x.Id == model.RoleId).Select(x => x.Name).FirstOrDefaultAsync();
    ...
    {
        return await _db.Movies.Include(x => x.SubCategory).FirstOrDefaultAsync(x => x.Id == id) ?? new Movie { };
    }
    ...
    public async Task<IEnumerable<MovieLink>> GetMovieLinksAsync(string search)
    {
        ...
        else
        {
            ...
            return await _db.MovieLinks.Include(x => x.Movie).ThenInclude(x => x.SubCategory)
                    .Where(x => x.Movie.MovieName.ToLower().Contains(search) || x.MLink.ToLower().Contains(search))
                    .ToListAsync();
        }
    ...
    {
        return await _db.MovieLinks.FirstOrDefaultAsync(x => x.Id == id) ?? new MovieLink { };
    }
    ...
    public async Task<bool> EditMovieLinkAsync(MovieLink movieLink, IFormFile video)
    {
        ...
        {
            if (movLink.MLink.ToLower() != video.FileName.ToLower())
            {
                // Real path
        ...
        if (video != null && video.Length > 0)
        {
            if (movLink.MLink.ToLower() != video.FileName.ToLower())
            {
                movLink.MLink = video.FileName;
                _db.Entry(movLink).Property(x => x.MLink).IsModified = true;
            }
        }
        else
        {
            movLink.MLink = movieLink.MLink;
            _db.Entry(movLink).Property(x => x.MLink).IsModified = true;
        }
    ...
    {
        return await _db.MovieActors.FirstOrDefaultAsync(x => x.Id == id) ?? new MovieActor { };
    }
